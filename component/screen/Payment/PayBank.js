...
export default class PayBank extends Component {
    ...
    this.state = {
      ...
      fullName:
        this.props.navigation.state.params.allData.allDataInfo.allinfoData.data
          .infodata.data.userDa.firstname,
      lastname:
        this.props.navigation.state.params.allData.allDataInfo.allinfoData.data
  ...
  _confirmBooking = () => {
    if (this.state.tran == '' || this.state.bank == '') {
      ...
      axios
        .get(
          `${this.state.settings.base_url}Api/BankBooking?booking_id_no=${this.state.allDataInfo.allDataInfo.bookid}&bank_id=${this.state.bank}&tran_num=${this.state.tran}`,
        )
        .then(res => {
          Alert.alert(
            this.state.settings.settings.title,
